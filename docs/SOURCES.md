...
| careerjet_ph | 1 | PH publisher JSON API (Basic auth, locale en_PH, keywords=remote) | 6h | implemented — live 403: `Unauthorized access from IP` (publisher key is IP-bound; whitelist current egress IP in publisher account) | per-request user_ip required → server egress IP via cached api.ipify.org lookup (`GIGGREGATOR_CAREERJET_USER_IP` override); LOCATIONS-type responses = no jobs; structured salary re-serialized to raw pay string |
| jooble_ph | 1 | PH jobs API (POST, keywords=remote, location=Philippines) | 24h | **live-verified 2026-09-16** (20 jobs, fixture committed) | free quota is 500 lifetime requests/key → one small page (20) per cycle; salary passed through raw, empty salary → body-scan fallback |
